...
class MetaCorgiSnacks
  ...
  #   # Your code goes here...
  #   name.to_s + ": " + @snack_box.send("get_#{name}_info", @box_id) + " " + @snack_box.send("get_#{name}_tastiness", @box_id).to_s
  # end


  def self.define_snack(name)

    define_method("get_#{name}_info") do |box_id| # or even |*args|
      return @snack_box[box_id][name].info
    end

    define_method("get_#{name}_tastiness") do |box_id| # or even |*args|
      return @snack_box[box_id][name].tastiness
    end

    define_method("#{name}") do |s| # or even |*args|
      return "#{name}: " + self.send("get_#{name}_info") + ":" + self.send("get_#{name}_tastiness")
    end
